...
impl std::fmt::Display for NoDbName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "The database name was None")
    }
}

#[derive(Debug, Clone)]
struct NoDbConnection;

impl std::fmt::Display for NoDbConnection {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "The database connection was None")
    }
}

#[derive(Error, Debug)]
pub enum CheckError {
    #[error("The database name was None")]
    NoDbName,

    #[error("The database connection was None")]
    NoDbConnection
}
...
pub fn check(db_name: &Option<String>, connection: &mut Option<PooledConn>) -> Result<Check, CheckError> {
    ...
    if db_name.is_none() {
        return Err(CheckError::NoDbName);
    }
    ...
    if let Some(conn) = connection.as_mut() {
        ...
        return Err(CheckError::NoDbConnection);
    }
