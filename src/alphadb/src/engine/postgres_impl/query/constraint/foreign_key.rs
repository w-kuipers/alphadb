...
/// Generate PostgreSQL FOREIGN KEY table constraints.
///
...
/// ]
/// ```
pub fn create_foreign_key_constraints(foreign_keys: &Value, table_name: &str, version_number: &str) -> Result<Vec<String>, AlphaDBError> {
    let foreign_keys = foreign_keys.as_array().ok_or_else(|| AlphaDBError {
    ...
    foreign_keys
        .iter()
        .map(|foreign_key| create_foreign_key_constraint(foreign_key, table_name, version_number))
        .collect()
}

fn create_foreign_key_constraint(foreign_key_value: &Value, table_name: &str, version_number: &str) -> Result<String, AlphaDBError> {
...
#[cfg(test)]
mod createforeignkeyconstraints_tests {
    use super::create_foreign_key_constraints;
    use serde_json::json;

    #[test]
    fn missing_from() {
        let foreign_key = json!([{ "references": "test" }]);
        let result = create_foreign_key_constraints(&foreign_key, "table", "0.0.1");
    ...
    #[test]
    fn invalid_foreign_key_array() {
        let foreign_key = json!({ "from": "test" });
        let result = create_foreign_key_constraints(&foreign_key, "table", "0.0.1");

        assert!(result.is_err());
        assert_eq!(result.unwrap_err().message, "foreign_key must be an array of objects");
    }
    ...
    fn foreign_key_constraint() {
        ...
        let result = create_foreign_key_constraints(&foreign_key, "table", "0.0.1").unwrap();

        assert_eq!(result, vec!["FOREIGN KEY (key) REFERENCES other_table (key) ON DELETE CASCADE ON UPDATE RESTRICT"]);
    }
}
